...
const MainComponent = () => {
  // cart is an array which will store the products we are interested in buying
  const [cart, setCart] = useState([]);

  // cartTotal is the amount of money pay
  ...
  // Handler to cycle through views and loop back after the last view.
  const changeView = () => {
    setCurrentView1((prevView) => (prevView % 2) + 1);
  };

  // Dynamically set the button content based on the current view
  let buttonContent = "";
  switch (currentView1) {
    case 1:
      buttonContent = "Cart";
  ...
  //dataF Hook to contain the input data.
  const [formData, setFormData] = useState({});

  //Search input query
  const [query, setQuery] = useState("");

  //render the products category being affected by search
    ...
    <div className="col mb-4" key={el.id}>
      <div className="card shadow-sm">
        <img
          src={el.image}
          alt={el.title}
          className="card-img-top" // Bootstrap class for image styling
